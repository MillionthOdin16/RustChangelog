...
public static class ServerOcclusion
{
	public readonly struct Grid : IEquatable<Grid>
	{
		...
		{
			return HashCode.Combine<int, int, int>(x, y, z);
		}
	...
	public static bool PathBetweenDirect(SubGrid start, SubGrid end)
	{
		...
		int num = Mathf.Abs(Mathf.RoundToInt((end.GetCenterPoint().x - start.GetCenterPoint().x) / 2f));
		int num2 = Mathf.Abs(Mathf.RoundToInt((end.GetCenterPoint().y - start.GetCenterPoint().y) / 2f));
		int num3 = Mathf.Abs(Mathf.RoundToInt((end.GetCenterPoint().z - start.GetCenterPoint().z) / 2f));
		int num4 = Mathx.Max(num, num2, num3);
		for (int i = 0; i < num4; i++)
		{
			float num5 = (float)i / (float)(num4 - 1);
			if (IsBlocked(GetSubGrid(Vector3.LerpUnclamped(start.GetCenterPoint(), end.GetCenterPoint(), num5))))
			{
				return false;
			}
		}
