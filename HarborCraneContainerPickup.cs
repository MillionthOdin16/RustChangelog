...
public class HarborCraneContainerPickup : HarborCrane
{
	...
	public override void ServerInit()
	{
		base.ServerInit();
		AllCranes.Add(this);
		List<HarborCraneContainerSpawnPoint> list = Pool.GetList<HarborCraneContainerSpawnPoint>();
		...
		Pool.FreeList<HarborCraneContainerSpawnPoint>(ref list);
		UpdateArm();
		SendNetworkUpdate();
	}
	...
	private void ReplenishContainers(int totalContainers)
	{
		...
		if (count == 0)
		{
			Debug.Log((object)"Can't spawn containers, no stacks...");
			return;
		}
		...
		while (totalContainers > 0 && num2 < 50)
		{
			...
			{
				Debug.Log((object)"Failed container spawn, trying again...");
			}
		}
		movesToMake.Reverse();
		bool SpawnContainerOnStack(ContainerStack stack)
		{
			...
			if (CargoShip.docking_debug)
			{
				Debug.Log((object)"Successfully spawned container");
			}
			return true;
		}
	}
	...
	public override void Save(SaveInfo info)
	{
		...
		{
			info.msg.harborCrane.queuedMoves = Pool.GetList<QueuedMove>();
	...
	public void AssignDestination(List<Transform> destinations, CargoShip targetShip, float delay)
	{
		...
		moveDelay = delay;
		Debug.Log((object)$"Crane delay: {delay}s");
		CheckMoveQueue();
	}
	...
	public override void Load(LoadInfo info)
	{
		...
		if (info.msg.harborCrane.queuedMoves != null)
		{
			...
			Pool.FreeList<QueuedMove>(ref info.msg.harborCrane.queuedMoves);
		}
