...
public class BaseMountable : BaseCombatEntity
{
	...
	public virtual bool ValidDismountPosition(BasePlayer player, Vector3 disPos)
	{
		...
		if (!Physics.CheckCapsule(val, val2, 0.5f, 1537286401))
		{
			...
			if (IsVisible(position))
			{
				...
				if (!AntiHack.TestNoClipping(dismountCheckStart, val3, player.NoClipRadius(ConVar.AntiHack.noclip_margin_dismount), ConVar.AntiHack.noclip_backtracking, sphereCast: true, vehicleLayer: false, legacyDismount ? null : this))
				{
					if (debugDismounts)
					{
	...
	public virtual void AttemptMount(BasePlayer player, bool doMountChecks = true)
	{
		...
		if (doMountChecks)
		{
			if (checkPlayerLosOnMount && Physics.Linecast(player.eyes.position, mountAnchor.position + ((Component)this).transform.up * 0.5f, 1218652417))
			{
	...
	public void DismountPlayer(BasePlayer player, bool lite = false)
	{
		...
		else
		{
			...
			if (Object.op_Implicit((Object)(object)player.GetParentEntity()))
			{
				...
				player.ClientRPCPlayer<Vector3, uint>(null, player, "ForcePositionToParentOffset", ((Component)baseEntity).transform.InverseTransformPoint(res), baseEntity.net.ID);
			}
			else
			{
