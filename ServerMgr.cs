...
public class ServerMgr : SingletonComponent<ServerMgr>, IServerCallback
{
	...
	private void UpdateServerInformation()
	{
		...
		try
		{
			...
			SteamServer.GameTags = $"mp{ConVar.Server.maxplayers},cp{BasePlayer.activePlayerList.Count},pt{Net.sv.ProtocolId},qp{SingletonComponent<ServerMgr>.Instance.connectionQueue.Queued},v{2377}{text4}{text6},h{AssemblyHash},{text},{text2},{text3}";
	...
	private void OnGiveUserInformation(Message packet)
	{
		...
		{
			DebugEx.Log((object)("Kicking " + ((object)packet.connection)?.ToString() + " - their protocol is " + packet.connection.protocol + " not " + 2377), (StackTraceLogType)0);
