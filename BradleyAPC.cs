...
public class BradleyAPC : BaseCombatEntity
{
	...
	public void UpdateMovement_Patrol()
	{
		...
		if ((Object)(object)patrolPath == (Object)null || Time.time < nextPatrolTime)
		{
			return;
		}
		...
		PathInterestNode randomInterestNodeAwayFrom = patrolPath.GetRandomInterestNodeAwayFrom(((Component)this).transform.position);
		BasePathNode closestToPoint = patrolPath.GetClosestToPoint(((Component)randomInterestNodeAwayFrom).transform.position);
		...
		{
			basePathNode = patrolPath.GetClosestToPoint(((Component)this).transform.position);
		}
		if (!(Vector3.Distance(finalDestination, ((Component)closestToPoint).transform.position) > 2f))
		{
			return;
		}
		...
		{
			currentPath.Clear();
			currentPath.Add(((Component)closestToPoint).transform.position);
			currentPathIndex = -1;
			pathLooping = false;
			finalDestination = ((Component)closestToPoint).transform.position;
		}
		else
		{
			...
			if (flag)
			{
				...
				{
					currentPath.Add(((Component)nodes[i]).transform.position);
				}
			}
			foreach (BasePathNode item in path)
			{
				currentPath.Add(((Component)item).transform.position);
			}
			currentPathIndex = -1;
			pathLooping = false;
			finalDestination = ((Component)closestToPoint).transform.position;
	...
	public void UpdateMovement_Hunt()
	{
		...
		else
		{
			...
			bool flag = false;
			BasePathNode start = patrolPath.GetClosestToPoint(((Component)this).transform.position);
			List<BasePathNode> nodes = Pool.GetList<BasePathNode>();
			if (GetEngagementPath(ref nodes))
			{
				flag = true;
			...
			foreach (BasePathNode item2 in nearNodes)
			{
				...
				if (targetInfo.entity.IsVisible(((Component)item2).transform.position + new Vector3(0f, y, 0f)) && AStarPath.FindPath(start, item2, out path, out var pathCost) && pathCost < num)
				{
					stack = path;
					num = pathCost;
					basePathNode = item2;
			...
			if (stack != null)
			{
				...
				{
					currentPath.Add(((Component)item3).transform.position);
				}
				currentPathIndex = -1;
				pathLooping = false;
				finalDestination = ((Component)basePathNode).transform.position;
			}
	...
	public Vector3 PathDirection(int index)
	{
		...
		{
			zero = ((index - 1 >= 0) ? currentPath[index - 1] : ((Component)this).transform.position);
	...
	public bool GetPathToClosestTurnableNode(BasePathNode start, Vector3 forward, ref List<BasePathNode> nodes)
	{
		...
		{
			val = ((Component)item).transform.position - ((Component)start).transform.position;
		...
		if ((Object)(object)basePathNode != (Object)null)
		{
			...
			val = ((Component)basePathNode).transform.position - ((Component)start).transform.position;
	...
	public bool GetEngagementPath(ref List<BasePathNode> nodes)
	{
		...
		//IL_0046: Unknown result type (might be due to invalid IL or missing references)
		//IL_006d: Unknown result type (might be due to invalid IL or missing references)
		BasePathNode closestToPoint = patrolPath.GetClosestToPoint(((Component)this).transform.position);
		Vector3 val = ((Component)closestToPoint).transform.position - ((Component)this).transform.position;
		Vector3 normalized = ((Vector3)(ref val)).normalized;
	...
	public void UpdateTargetList()
	{
		...
		for (int num = targetList.Count - 1; num >= 0; num--)
		{
			...
			if ((Object)(object)targetInfo2.entity == (Object)null || Time.time - targetInfo2.lastSeenTime > memoryDuration || basePlayer2.IsDead())
			{
