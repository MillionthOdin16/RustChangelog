...
public class PlayerInventory : EntityComponent<BasePlayer>
{
	...
	public void MoveItem(BaseEntity.RPCMessage msg)
	{
		...
		if (item == null)
		{
			msg.player.ChatMessage("Invalid item (" + id + ")");
			return;
		}
		...
		if (num == 0)
		{
			...
			{
				baseEntity = (((Object)(object)entityOwner == (Object)(object)base.baseEntity) ? loot.entitySource : base.baseEntity);
			}
			if (baseEntity is IIdealSlotEntity idealSlotEntity)
			{
				num = idealSlotEntity.GetIdealContainer(base.baseEntity, item);
			}
		...
		{
			msg.player.ChatMessage("Invalid container (" + num + ")");
			return;
		}
