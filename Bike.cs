...
public class Bike : GroundVehicle, VehicleChassisVisuals<Bike>.IClientWheelUser, IPrefabPreProcess, CarPhysics<Bike>.ICar, TriggerHurtNotChild.IHurtTriggerUser
{
	...
	[SerializeField]
	[Range(0f, 1f)]
	[FormerlySerializedAs("twoWheelPitchStabP")]
	private float pitchStabP = 0.01f;

	[SerializeField]
	[Range(0f, 1f)]
	[FormerlySerializedAs("twoWheelPitchStabD")]
	...
	private void PDRollStab(float speed)
	{
		...
		float num = 0f - SteerAngle / MaxSteerAngle * Mathf.Clamp01(speed / maxLeanSpeed);
		num = ((!(num < 0f)) ? (num * leftMaxLean) : (num * rightMaxLean));
		float num2 = ((Component)this).transform.localEulerAngles.z;
		if (num2 > 180f)
		{
			num2 -= 360f;
		}
		float num3 = num - num2;
		float num4 = num3;
		float num5 = 0f - AngleDifference(num2, prevRollStabRoll) / Time.fixedDeltaTime;
		float num6 = twoWheelRollStabP * num4 + twoWheelRollStabD * num5;
		rigidBody.AddRelativeTorque(0f, 0f, num6, (ForceMode)2);
		prevRollStabError = num3;
		prevRollStabRoll = num2;
	}
	...
	public override Vector3 GetMountRagdollVelocity(BasePlayer player)
	{
		...
		Mathf.Clamp(lastCrashDamage, 0f, 75f);
		return ((Component)this).transform.forward * lastCrashDamage * 0.5f;
	}
