...
public class WaterRadialMesh
{
	...
	private Mesh[] GenerateMeshes(int vertexCount, bool volume = false)
	{
		...
		for (int i = 0; i < num; i++)
		{
			...
			array[num7] = ((Vector2)(ref val)).normalized;
		}
	...
	private Vector3 RaycastPlane(Camera camera, float planeHeight, Vector3 pos)
	{
		...
		if (((Component)camera).transform.position.y > planeHeight)
		{
			if (((Ray)(ref val)).direction.y > -0.01f)
			{
				((Ray)(ref val)).direction = new Vector3(((Ray)(ref val)).direction.x, 0f - ((Ray)(ref val)).direction.y - 0.02f, ((Ray)(ref val)).direction.z);
			}
		}
		else if (((Ray)(ref val)).direction.y < 0.01f)
		{
			((Ray)(ref val)).direction = new Vector3(((Ray)(ref val)).direction.x, 0f - ((Ray)(ref val)).direction.y + 0.02f, ((Ray)(ref val)).direction.z);
		}
		float num = (0f - (((Ray)(ref val)).origin.y - planeHeight)) / ((Ray)(ref val)).direction.y;
		return Quaternion.AngleAxis(0f - ((Component)camera).transform.eulerAngles.y, Vector3.up) * (((Ray)(ref val)).direction * num);
	}
	...
	public Matrix4x4 ComputeLocalToWorldMatrix(Camera camera, float oceanWaterLevel)
	{
		...
		Vector3 val = ((Matrix4x4)(ref worldToCameraMatrix)).MultiplyVector(Vector3.up);
		worldToCameraMatrix = camera.worldToCameraMatrix;
		Vector3 val2 = ((Matrix4x4)(ref worldToCameraMatrix)).MultiplyVector(Vector3.Cross(((Component)camera).transform.forward, Vector3.up));
		Vector3 val3 = new Vector3(val.x, val.y, 0f);
		val = ((Vector3)(ref val3)).normalized * 0.5f + new Vector3(0.5f, 0f, 0.5f);
		val3 = new Vector3(val2.x, val2.y, 0f);
		val2 = ((Vector3)(ref val3)).normalized * 0.5f;
		...
		val6.x = num * Mathf.Tan(camera.fieldOfView * 0.5f * ((float)Math.PI / 180f)) * camera.aspect + 2f;
		val6.y = num;
		val6.z = num;
		...
		Quaternion val7 = Quaternion.AngleAxis(Mathf.Atan2(forward.x, forward.z) * 57.29578f, Vector3.up);
