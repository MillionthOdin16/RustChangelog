...
public class MotorRowboat : BaseBoat
{
	...
	public void UpdateDrag()
	{
		...
		rigidBody.angularDrag = angularDragBase + angularDragVelocity * num2;
		rigidBody.drag = landDrag + waterDrag * Mathf.InverseLerp(0f, 1f, buoyancy.submergedFraction);
		if (offAxisDrag > 0f)
		{
			Vector3 forward = ((Component)this).transform.forward;
			Vector3 velocity = rigidBody.velocity;
			float num3 = Vector3.Dot(forward, ((Vector3)(ref velocity)).normalized);
			float num4 = Mathf.InverseLerp(0.98f, 0.92f, num3);
			Rigidbody obj = rigidBody;
			obj.drag += num4 * offAxisDrag * buoyancy.submergedFraction;
		}
	}
