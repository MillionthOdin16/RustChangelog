...
public class BaseVehicle : BaseMountable
{
	...
	public override Quaternion GetAngularVelocityServer()
	{
		...
		if (((Vector3)(ref angularVelocity)).sqrMagnitude < 0.025f)
		{
			return Quaternion.identity;
		}
		return Quaternion.LookRotation(rigidBody.angularVelocity, ((Component)this).transform.up);
	}
	...
	public virtual bool IsSeatClipping(BaseMountable mountable)
	{
		...
		if (checkVehicleClipping)
		{
			...
			foreach (Collider item in list)
			{
				...
				if ((Object)(object)baseEntity != (Object)(object)this && !EqualNetID(baseEntity))
				{
					result = true;
					break;
				}
	...
	public void SwapSeats(BasePlayer player, int targetSeat = 0)
	{
		...
		if ((Object)(object)baseMountable == (Object)null)
		{
			...
			for (int i = 0; i < num2; i++)
			{
				...
				if ((Object)(object)mountPoint?.mountable != (Object)null && !mountPoint.mountable.AnyMounted() && mountPoint.mountable.CanSwapToThis(player) && !IsSeatClipping(mountPoint.mountable) && IsSeatVisible(mountPoint.mountable, player.eyes.position))
				{
