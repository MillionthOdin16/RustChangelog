...
public class PerformanceLogging
{
	...
	public void Flush()
	{
		...
		using (SHA256 sHA = SHA256.Create())
		{
			...
			value2 = Convert.ToBase64String(sHA.ComputeHash(uTF.GetBytes(((object)(SteamId)(ref steamId)).ToString() + "random_junk_here_489327534")));
		}
		if (isClient)
		{
		...
		{
			["unity"] = Application.unityVersion ?? "editor",
	...
	private void ResetMeasurements()
	{
		...
		if (Frametimes.Count != 0)
		{
			...
			{
				Pool.Free<List<TimeSpan>>(ref result.Frametimes);
				Pool.Free<List<int>>(ref result.Ping);
			}
