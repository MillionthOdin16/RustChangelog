...
public class TravellingVendor : BaseEntity, VehicleChassisVisuals<TravellingVendor>.IClientWheelUser
{
	...
	private Dictionary<BasePlayer, float> ignoreList;

	private Dictionary<BasePlayer, VendorTargetInfo> possiblePatrons;

	private Dictionary<BasePlayer, float> blockerPlayers;

	private List<BasePlayer> ignoreDebugList = new List<BasePlayer>();

	private List<BasePlayer> possiblePatronsDebugList = new List<BasePlayer>();

	private List<BasePlayer> blockerDebugList = new List<BasePlayer>();

	private int searchRange = 10;

	private float memoryDuration = 5f;

	private float allowedVendorBlockTime = 45f;
	...
	private static (Vector3, int) FindFarthestPointFromPlayers(Vector3[] points, List<BasePlayer> players)
	{
		...
		for (int i = 0; i < points.Length; i++)
		{
			...
			{
				if (Object.op_Implicit((Object)(object)player) && player.IsConnected && !player.IsInTutorial)
				{
					float num3 = Vector3.Distance(val, ((Component)player).transform.position);
					if (num3 < num2)
					{
						num2 = num3;
					}
	...
	public static TravellingVendor SpawnTravellingVendorForEvent()
	{
		...
		List<BasePlayer> players = new List<BasePlayer>((IEnumerable<BasePlayer>)BasePlayer.activePlayerList);
		var (val, num6) = FindFarthestPointFromPlayers(pathList.Path.Points, players);
		if (val == Vector3.zero)
		{
			Debug.Log((object)"Unable to find a suitable spawn point for travelling vendor. Picking randomly.");
			num6 = Random.Range(0, pathList.Path.Points.Length - 1);
		}
		bool flag = CheckSpawnPosition(pathList.Path.Points[num6]);
		if (!flag)
		{
			for (int k = 0; k < 5; k++)
			{
				num6 = Random.Range(0, pathList.Path.Points.Length - 1);
				flag = CheckSpawnPosition(pathList.Path.Points[num6]);
				if (flag)
				{
					break;
				}
		...
		if (flag)
		{
			Vector3 val2 = pathList.Path.Points[(num6 + 1) % pathList.Path.Points.Length] - pathList.Path.Points[num6];
			Vector3 normalized = ((Vector3)(ref val2)).normalized;
			BaseEntity baseEntity = GameManager.server.CreateEntity("assets/prefabs/npc/travelling vendor/travellingvendor.prefab", pathList.Path.Points[num6] + Vector3.up * 2f, Quaternion.LookRotation(normalized));
			TravellingVendor travellingVendor = null;
			if (Object.op_Implicit((Object)(object)baseEntity))
			{
				travellingVendor = ((Component)baseEntity).GetComponent<TravellingVendor>();
				...
				{
					travellingVendor.Spawn();
					travellingVendor.InstallPath(runtimePath, (num6 + 1) % pathList.Path.Points.Length);
				}
				else
				{
					baseEntity.Kill();
	...
	{
		//IL_0000: Unknown result type (might be due to invalid IL or missing references)
		int num = Physics.OverlapSphereNonAlloc(testPosition, 0.2f, spawncheckColliders, obstacleMask);
		_ = 0;
		return num == 0;
	}
	...
	public void CreatePreventBuilding()
	{
		//IL_0029: Unknown result type (might be due to invalid IL or missing references)
		//IL_002e: Unknown result type (might be due to invalid IL or missing references)
		if (Object.op_Implicit((Object)(object)preventBuildingInstance))
		{
	...
	public override void Save(SaveInfo info)
	{
		...
		info.msg.travellingVendor.steeringAngleLeft = wheelFL.wheelCollider.steerAngle;
		info.msg.travellingVendor.steeringAngleRight = wheelFR.wheelCollider.steerAngle;
		info.msg.travellingVendor.velocity = myRigidbody.velocity;
	...
	public override void ServerInit()
	{
		...
		if (!base.isClient)
		{
			...
			obstacleHits = Pool.GetList<RaycastHit>();
			pulloverHits = Pool.GetList<RaycastHit>();
			...
			((FacepunchBehaviour)this).InvokeRandomized((Action)NightCheck, 0f, 180f, 30f);
		}
	}
	...
	internal override void DoServerDestroy()
	{
		...
		{
			Pool.FreeList<RaycastHit>(ref obstacleHits);
		}
		if (pulloverHits != null)
		{
			Pool.FreeList<RaycastHit>(ref pulloverHits);
		}
	}
			...
			if (!(item is Barricade barricade))
			{
				if (!(item is LootContainer lootContainer))
				{
					if (!(item is TreeEntity treeEntity))
					{
						...
						{
							if (item is TrainCar trainCar && trainCar.isServer && trainCar.IsAlive())
							{
	...
	private bool CheckForObstacle()
	{
		...
		foreach (RaycastHit obstacleHit in obstacleHits)
		{
			...
			if (!(((RaycastHit)(ref current)).collider.ToBaseEntity() is BasePlayer basePlayer) || ignoreList.ContainsKey(basePlayer) || basePlayer.IsFlying || IsInvalidPlayer(basePlayer))
			{
				continue;
			}
			ClientRPC(RpcTarget.NetworkGroup("CL_PlayerDetected"));
			if (!blockerPlayers.TryAdd(basePlayer, Time.time))
			{
				float num = blockerPlayers[basePlayer];
				if (Time.time - num >= 60f)
				{
					num = Time.time;
					blockerPlayers[basePlayer] = num;
				}
				if (Time.time - num > allowedVendorBlockTime)
				{
					IgnorePlayer(basePlayer);
					blockerPlayers.Remove(basePlayer);
				}
			}
			obstacleHits.Clear();
			return true;
	...
	private void UpdateObstacleList(List<RaycastHit> hits, Vector3 forward, float checkRadius = 2.5f)
	{
		...
		//IL_0043: Unknown result type (might be due to invalid IL or missing references)
		hits.Clear();
		GamePhysics.TraceAll(new Ray(((Component)this).transform.position + ((Component)this).transform.forward * (((Bounds)(ref bounds)).extents.z / 0.5f - 1f), forward), checkRadius, hits, 15f, obstacleMask | 1 | 0x8000, (QueryTriggerInteraction)1, this);
	}
	...
	private void ApplyForceAtWheels()
	{
		...
		Vector3 velocity = myRigidbody.velocity;
		bool flag = ((Vector3)(ref velocity)).magnitude >= maxSpeed;
	...
	public void InstallPath(RuntimePath path, int initialDestination)
	{
		...
		if (currentPath == null)
		{
			currentPath = Pool.GetList<Vector3>();
		}
		currentPath.Clear();
	...
	{
		if (possiblePatrons == null)
		{
			possiblePatrons = new Dictionary<BasePlayer, VendorTargetInfo>();
		}
		if (ignoreList == null)
		{
			ignoreList = new Dictionary<BasePlayer, float>();
		}
		if (blockerPlayers == null)
		{
			blockerPlayers = new Dictionary<BasePlayer, float>();
		}
		List<BaseEntity> list = Pool.GetList<BaseEntity>();
		FetchCycle(list);
		if (list.Count == 0 && possiblePatrons.Count != 0)
		{
			possiblePatrons.Clear();
		}
		Pool.FreeList<BaseEntity>(ref list);
	}
	...
	private void FetchCycle(List<BaseEntity> foundEntities)
	{
		...
		BasePlayer basePlayer2 = default(BasePlayer);
		VendorTargetInfo vendorTargetInfo = default(VendorTargetInfo);
		foreach (BaseEntity foundEntity in foundEntities)
		{
			...
			bool flag = true;
			for (int num = possiblePatrons.Count - 1; num >= 0; num--)
			{
				possiblePatrons.ElementAt(num).Deconstruct(ref basePlayer2, ref vendorTargetInfo);
				BasePlayer basePlayer3 = basePlayer2;
				VendorTargetInfo vendorTargetInfo2 = vendorTargetInfo;
				if (ignoreList.TryGetValue(basePlayer3, out var value))
				{
					if (Time.time > value)
					{
						ignoreList.Remove(basePlayer3);
					}
					else
					{
						flag = false;
				...
				if ((ulong)basePlayer.userID == (ulong)basePlayer3.userID)
				{
					VendorTargetInfo value2 = possiblePatrons[basePlayer3];
					value2.lastSeenTime = Time.time;
					possiblePatrons[basePlayer3] = value2;
					flag = false;
				}
				if (false | (Time.time - vendorTargetInfo2.lastSeenTime > memoryDuration) | IsInvalidPlayer(basePlayer3))
				{
					possiblePatrons.Remove(basePlayer3);
				}
			}
			if (ignoreList.ContainsKey(basePlayer))
			{
				if (possiblePatrons.ContainsKey(basePlayer))
				{
					possiblePatrons.Remove(basePlayer);
				}
				flag = false;
			}
			else if (flag)
			{
				Dictionary<BasePlayer, VendorTargetInfo> dictionary = possiblePatrons;
				vendorTargetInfo = new VendorTargetInfo
				{
					lastSeenTime = Time.time
				};
				dictionary.Add(basePlayer, vendorTargetInfo);
			}
		}
	}
	...
	{
		float num = 90f;
		ignoreList[player] = Time.time + num;
	}

	private bool IsValidPatrons()
	{
		Dictionary<BasePlayer, VendorTargetInfo> dictionary = possiblePatrons;
		if ((dictionary != null && dictionary.Count == 0) || possiblePatrons == null)
		{
			return false;
		}
		bool result = false;
		foreach (KeyValuePair<BasePlayer, VendorTargetInfo> possiblePatron in possiblePatrons)
		{
			if (!ignoreList.ContainsKey(possiblePatron.Key) && !blockerPlayers.ContainsKey(possiblePatron.Key))
			{
				result = true;
				break;
			}
