...
public class TravellingVendor : BaseEntity, VehicleChassisVisuals<TravellingVendor>.IClientWheelUser
{
	...
	private static int FindRandomPointIndexBeyondThreshold(Vector3[] points, List<BasePlayer> players, float distanceThreshold, List<int> validIndices)
	{
		...
		{
			int index = Random.Range(0, validIndices.Count);
	...
	public static TravellingVendor SpawnTravellingVendorForEvent()
	{
		...
		int num6 = FindRandomPointIndexBeyondThreshold(pathList.Path.Points, players, 10f, list);
		...
		if (flag)
		{
			Vector3 val = pathList.Path.Points[(num6 + 1) % pathList.Path.Points.Length] - pathList.Path.Points[num6];
	...
	internal override void DoServerDestroy()
	{
		...
		{
			Pool.FreeList<BasePlayer>(ref localPlayers);
		}
		...
		if (currentPath != null)
		{
			Pool.FreeList<Vector3>(ref currentPath);
		}
	}
	...
	private void FetchTargets()
	{
		...
		if (localPlayers == null)
		{
			localPlayers = Pool.GetList<BasePlayer>();
		}
		List<BaseEntity> list = Pool.GetList<BaseEntity>();
		FetchCycle(list);
		Pool.FreeList<BaseEntity>(ref list);
