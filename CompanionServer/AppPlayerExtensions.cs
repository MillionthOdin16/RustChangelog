...
public static class AppPlayerExtensions
{
	...
	public static AppTeamInfo GetAppTeamInfo(this RelationshipManager.PlayerTeam team, ulong requesterSteamId)
	{
		...
		for (int i = 0; i < team.members.Count; i++)
		{
			...
			Vector2 val2 = Util.WorldToMap((basePlayer2 != null) ? ((Component)basePlayer2).transform.position : Vector3.zero);
			...
			val3.spawnTime = (basePlayer2?.lifeStory?.timeBorn).GetValueOrDefault();
			val3.isAlive = basePlayer2?.IsAlive() ?? false;
			val3.deathTime = (basePlayer2?.previousLifeStory?.timeDied).GetValueOrDefault();
