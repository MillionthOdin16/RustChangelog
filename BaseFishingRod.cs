	...
	private bool EvaluateFishingPosition(ref Vector3 pos, BasePlayer ply, out FailReason reason, out WaterBody waterBody)
	{
		...
		if (list.Count > 0)
		{
			foreach (Collider item in list)
			{
				...
				{
					waterBody = ((Component)((Component)item).transform.parent).GetComponentInChildren<WaterBody>();
				}
				...
				flag = true;
				if (GamePhysics.Trace(new Ray(pos + Vector3.up, -Vector3.up), 0f, out var hitInfo, 1.5f, 16, (QueryTriggerInteraction)2))
				{
					pos.y = ((RaycastHit)(ref hitInfo)).point.y;
				}
				else
				{
					pos.y = ((Component)list[0]).transform.position.y;
				}
				if (!waterBody.IsOcean)
				{
					...
					{
						pos.y = ((Component)waterBody.Renderer).transform.position.y;
					}
					break;
				}
			}
		...
		Vector3 p2 = val + ((Vector3)(ref val2)).normalized * (num - FishCatchDistance);
		if (!GamePhysics.LineOfSight(pos, p2, 1218652417))
		{
			reason = FailReason.Obstructed;
			return false;
		}
		if (WaterLevel.GetOverallWaterDepth(Vector3.Lerp(pos, Vector3Ex.WithY(((Component)ply).transform.position, pos.y), 0.95f), waves: true, null, noEarlyExit: true) < 0.1f && ply.eyes.position.y > 0f)
		{
