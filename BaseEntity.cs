...
public class BaseEntity : BaseNetworkable, IOnParentSpawning, IPrefabPreProcess
{
	...
	public bool IsOutside(Vector3 position)
	{
		...
		{
			flag = false;
			if (!Physics.Raycast(position, Vector3.up, ref val, 100f, 161546513))
			{
				continue;
			}
			...
			{
				if (((RaycastHit)(ref val)).point.y > position.y + 0.2f)
				{
					position = ((RaycastHit)(ref val)).point + Vector3.up * 0.05f;
				}
				else
				{
					position.y += 0.2f;
	...
	public virtual void DebugServer(int rep, float time)
	{
		...
		DebugText(((Component)this).transform.position + Vector3.up * 1f, $"{((net != null) ? net.ID : 0u)}: {((Object)this).name}\n{DebugText()}", Color.white, time);
	}
