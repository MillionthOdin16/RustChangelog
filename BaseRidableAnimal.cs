...
public class BaseRidableAnimal : BaseVehicle
{
	...
	public void UpdateGroundNormal(bool force = false)
	{
		...
		if (Time.time >= nextGroundNormalUpdateTime || force)
		{
			...
			{
				if (TransformUtil.GetGroundInfo(array[i].position + Vector3.up * 2f, out var _, out var normal, 4f, LayerMask.op_Implicit(295763969)))
				{
					targetUp += normal;
				}
				else
			...
			if (flag)
			{
				...
				if (num8 > maxWallClimbSlope || num9 > maxWallClimbSlope)
				{
					...
					for (int j = 0; j < normalOffsets.Length; j++)
					{
						...
						if (TransformUtil.GetGroundInfo(val8 + Vector3.up * num12 + normalOffsets[j].z * ((Component)this).transform.forward, out var pos2, out var normal2, num7 + num12, LayerMask.op_Implicit(295763969)))
						{
							num11++;
							val7 += normal2;
							num10 += pos2.y;
