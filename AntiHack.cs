		...
		try
		{
			...
			if (ConVar.AntiHack.noclip_protection >= 3)
			{
				...
				while (ticks.MoveNext(num))
				{
					...
					if (TestNoClipping(val3 + val5, val4 + val5, radius, noclip_backtracking, sphereCast: true, vehicleLayer))
					{
						return true;
					}
					val3 = val4;
			...
			else if (TestNoClipping(val3 + val5, val4 + val5, radius, noclip_backtracking, sphereCast: false, vehicleLayer))
			{
				return true;
			}
			return false;
